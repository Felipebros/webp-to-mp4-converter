    ...
    try:
        with Image.open(input_path) as img:
            ...
            for frame in ImageSequence.Iterator(img):
                ...
                d = frame.info.get('duration', 40) # Padrão 40ms (25fps) se não encontrar
                durations.append(d)
                
                frame_rgb = frame.convert("RGB")
                if new_width != width or new_height != height:
                    frame_rgb = frame_rgb.resize((new_width, new_height), Image.Resampling.LANCZOS)
            ...
            print(f"Duração média detectada: {avg_duration:.2f}ms (FPS: {detected_fps:.2f})")
